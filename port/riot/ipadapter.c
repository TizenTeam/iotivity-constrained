...
oc_send_buffer(oc_message_t *message)
{
  ...
  conn_udp_sendto(message->data, message->length, local_addr, 16,
                  message->endpoint.addr.ipv6_addr.address, 16, AF_INET6,
                  OCF_SERVER_PORT_UNSECURED, message->endpoint.addr.ipv6_addr.port);
}
                        ...
                        uint16_t *port)
{
  ...
  if (message) {
    ...
    memcpy(message->endpoint.addr.ipv6_addr.address, addr, 16);
    message->endpoint.addr.ipv6_addr.port = *port;
